"""
  ...
  Jun-2019: modified by Alaa Akkoush- Calculation of vibrational modes for
  periodic systems and vibrational Raman intensities for periodic and non-
  periodic systems.
  August-2019: modified by Alaa Akkoush- Editing mode 2 and adding mode 3. 
  ...
  vibrational modes can be calculated (mode = 1). 
  Mode = 2 combines the two other modes (only usefule if aims-binary is provided). 
  Only the atoms without constrain_relaxation .true. in geometry.in will be displaced. 
  In mode 2, you have to specify whether you want Infrared, Raman or both calculations 
  to be done. 
  In mode 3, Hessian matrix, eigenvectors,  normal modes and their corresponding 
  frequencies are calculated and saved. 
  (Check python get_vibration.py --help for more details). 

  > get_vibrations.py [options] <name> <mode>
  > Example: 
  python get_vibrations.py -r path to aims excutable -x -p -w -d 0.025 material 2 -M > output
  ...
  {name}_IR_spectrum.pdf - IR spectrum
  {name}_Raman_spectrum.pdf - Raman spectrum
  eig_vec.{name}.dat - eigenvectors
  dynamical_matrix.{name}.dat - Mass weighted hessian metrix (Dynamical matrix)
...
will be replaced by name + counter, string 
  ...
  if options.plot or mode=='1' or mode=='2' or mode=='3':
  ...
  folder=''                                  # Dummy
  ########### Central Point ##################################################
  if options.relax and (mode=='0' or mode=='2' or mode=='3'):
  ...
  for atom in arange(n_atoms)[struc.constrained==False]:
    for coord in arange(3):
      for delta in deltas:
        ...
        if mode=='3':   # Put new geometry and control.in into folder
          struc_new=copy.deepcopy(struc)
          struc_new.atoms[atom].coord[coord]=struc_new.atoms[atom].coord[coord]+delta
          geoname='geometry.i_atom_'+str(atom)+'.i_coord_'+str(coord)+\
                  '.displ_'+str(delta)+'.in'
  ...
  if mode=='3': # Calculate vibrations
